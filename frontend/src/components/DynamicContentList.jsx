  ...
  return (
    <div className={`group glassmorphism hover-lift hover-spiritual-glow transition-all duration-500 overflow-hidden animate-fade-in-up card-${index + 1}`}>
      <div className="relative overflow-hidden">
        ...
        {item.language && (
          <div className="absolute top-4 right-4 bg-serene-blue text-white px-3 py-1 rounded-full text-sm font-medium font-primary">
      ...
      <div className="p-6">
        ...
        <Link to={linkPath}>
          <Button 
            variant="outline" 
            className={`w-full group-hover:bg-serene-blue group-hover:text-white group-hover:border-serene-blue transition-all duration-300 font-primary ${language === 'ar' ? 'flex-row-reverse' : ''}`}
          >
            <span>{language === 'ar' ? 'اقرأ المزيد' : 'Read More'}</span>
            <ArrowRight className={`h-4 w-4 group-hover:translate-x-1 transition-transform duration-300 ${language === 'ar' ? 'rotate-180 group-hover:-translate-x-1' : ''}`} />
          </Button>
        </Link>
      </div>
    </div>
