    ...
    <div className="min-h-screen py-12 bg-ivory-white">
      <div className="max-w-4xl mx-auto px-4">
        <h1 className="text-4xl font-bold text-deep-plum mb-8 font-heading">
          {t?.nav?.articles?.[language] || "Articles"}
        </h1>
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filtered.map((article) => (
              <div
                key={article.id}
                className="bg-white border border-gray-200 p-4 rounded-xl shadow-md transition-shadow hover:shadow-lg"
              >
                <h2 className="text-2xl font-semibold text-sky-teal font-heading mb-2">
